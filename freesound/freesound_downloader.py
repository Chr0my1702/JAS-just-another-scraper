...
def freesound_get_tokens_session_id(USER_NAME, USER_PASSWORD):
...
def download_getter(csrf_token, csrf_middleware_token, session_id, sound_username, sound_id, sound_title, USER_NAME, USER_PASSWORD):
    cookies = {
...
csrf_token, csrf_middleware_token, session_id = freesound_get_tokens_session_id(
    "", "")
sets = askdirectory(title='Select a directory with parquet sets')
save_folder = askdirectory(
    title='Select a directory to save the downloaded files')
